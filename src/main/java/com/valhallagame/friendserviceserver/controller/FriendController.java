...
public class FriendController {
	...
	public ResponseEntity<JsonNode> sendCharacterInvite(@RequestBody InviteCharacterParameter input)
			throws IOException {
		RestResponse<Character> charResp = characterServiceClient
				.getCharacterWithoutOwnerValidation(input.getReceiverCharacter());
		Optional<Character> charOpt = charResp.get();
	...
	public ResponseEntity<JsonNode> acceptCharacterInvite(@RequestBody AcceptCharacterParameter input)
		...
		Optional<Character> characterOpt = charResp.get();
	...
	private Optional<InviteData> convertToInviteData(Invite invite) throws IOException {
		...
		Optional<Person> senderPersonOpt = senderPersonResp.get();
		Optional<Character> senderCharacterOpt = senderCharacterResp.get();
		...
		Optional<Person> receiverPersonOpt = receiverPersonResp.get();
		Optional<Character> receiverCharacterOpt = receiverCharacterResp.get();

		if (senderPersonOpt.isPresent() && senderCharacterOpt.isPresent() && receiverPersonOpt.isPresent()
				&& receiverCharacterOpt.isPresent()) {
			Person senderPerson = senderPersonOpt.get();
			Character senderCharacter = senderCharacterOpt.get();

			Person receiverPerson = receiverPersonOpt.get();
			Character receiverCharacter = receiverCharacterOpt.get();
	...
	private Optional<FriendData> convertToFriendData(Friend friend) throws IOException {
		...
		Optional<Character> characterOpt = characterResp.get();
